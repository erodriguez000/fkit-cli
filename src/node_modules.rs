...
pub fn run(sub_cmd: &ArgMatches) -> Result<()> {
    match sub_cmd.subcommand() {
        Some(("get", _sub_cmd)) => {
            println!("{}", "Searching for node_module directories!".blue().bold());
            get();
        }
        Some(("delete", _sub_cmd)) => {
            println!("{}", "Searching and deleting node_module directories!".red().bold());
            _delete();
        }
...
fn get() {
    let child = Command::new("find")
        .args([".", "-name", "node_modules", "-type", "dir", "-prune"])
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()
        .unwrap();
