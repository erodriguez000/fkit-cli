...
use clap::ArgMatches;
use std::{str::from_utf8, path::Path, fs};
use colored::Colorize;
use crate::{exec::{prompt, spawn_and_wait}, utils};
use crate::prelude::*;
const DEFAULT_APP_NAME: &str = "react-native-app";

const FILES: &[&str; 1] = &[
    "app.json",
];

const GIT_TMPL_BASE: &str = "https://github.com/erodriguez000/react-native-app.git";
...
pub fn new(sub_cmd: &ArgMatches) -> Result<()> {
	...
	let prompt = "Creating your React Native app...".cyan();
    println!("{}", prompt);
    println!("{}", "...".cyan());
    println!("{}", "...".cyan());
    println!("{}", "...".cyan());
    ...
    spawn_and_wait(
		None,
		"git",
		&["clone", "--depth", "1", "--branch", "master", GIT_TMPL_BASE, &app_dir_name],
		false,
	)?;

    // --- Replace the parts
    let prompt = "Configuring your React Native app...".magenta();
    println!("{}", prompt);
    println!("{}", "...".magenta());
    println!("{}", "...".magenta());
    println!("{}", "...".magenta());
    ...
    let prompt = "React Native app initialized! Opening in VS Code".blue();
    println!("{}", prompt);
    println!("{}", "...".blue());
    println!("{}", "...".blue());
    println!("{}", "...".blue());
...
fn replace_parts(dir: &Path, conf: Conf) -> Result<()> {
    let files = FILES.iter().map(|f| utils::path_joins(dir, f)).collect::<Vec<_>>();
